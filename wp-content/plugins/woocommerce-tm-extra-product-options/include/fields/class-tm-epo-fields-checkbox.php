...
class THEMECOMPLETE_EPO_FIELDS_checkbox extends THEMECOMPLETE_EPO_FIELDS {
	...
	public function display_field( $element = array(), $args = array() ) {
		...
		} elseif ( ( ( THEMECOMPLETE_EPO()->is_quick_view() || empty( $this->post_data ) ) && empty( THEMECOMPLETE_EPO()->cart_edit_key ) ) || THEMECOMPLETE_EPO()->tm_epo_global_reset_options_after_add == "yes" ) {
			$selected_value = - 1;
		}
		...
		if ( $selected_value == - 1 ) {
			if ( ( THEMECOMPLETE_EPO()->is_quick_view() || empty( $this->post_data ) || THEMECOMPLETE_EPO()->tm_epo_global_reset_options_after_add == "yes" ) && isset( $default_value ) ) {
