...
class THEMECOMPLETE_EPO_FIELDS_radio extends THEMECOMPLETE_EPO_FIELDS {
	...
	public function display_field( $element = array(), $args = array() ) {
		...
		} elseif ( empty( $this->post_data ) || ! isset( $this->post_data[ $name ] ) || THEMECOMPLETE_EPO()->tm_epo_global_reset_options_after_add == "yes" ) {
			$selected_value = - 1;
		}
		...
		if ( $selected_value == - 1 ) {
			if ( ( empty( $this->post_data ) || ! isset( $this->post_data[ $name ] ) || THEMECOMPLETE_EPO()->tm_epo_global_reset_options_after_add == "yes" ) && isset( $default_value ) ) {
