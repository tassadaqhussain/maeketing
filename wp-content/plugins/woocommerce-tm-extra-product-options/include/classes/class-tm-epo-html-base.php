			...
			if ( empty( $args["nostart"] ) ) {
				...
				if ( empty( $args["nolabel"] ) && ! empty( $args["label"] ) ) {
					...
					if ( isset( $args["tags"] ) && isset( $args["tags"]["id"] ) ) {
						echo '<label for="' . esc_attr( $args["tags"]["id"] ) . '">';
					}
			...
			switch ( $args["type"] ) {
				...
				case "radio":
					foreach ( $args["options"] as $tx => $vl ) {
						...
						echo ' id="' . esc_attr( $args["tags"]["id"] ) . esc_attr( $tx ) . '"';
						echo ' type="radio" name="' . esc_attr( $args["tags"]["name"] ) . '" value="' . esc_attr( $vl["value"] ) . '"  />&nbsp;' . esc_html( $vl["text"] ) . '</label>';
						echo "<br />";
					}
					break;
				case "select":
