...
<tr class="tm-order-line-option item <?php echo esc_attr( $row_class ); ?>" data-order_item_id="<?php echo esc_attr( $item_id ); ?>" data-tm_item_id="<?php echo esc_attr( $item_id ); ?>" data-tm_key_id="<?php echo esc_attr( $key ); ?>">
    ...
    <td class="tm-c name" data-sort-value="<?php echo esc_attr( $item['name'] ); ?>">
		...
		<?php if ( $epo_edit_value ) { ?>
            <div class="edit">
                <div class="tm-50">
                    <textarea name="tm_epo[<?php echo esc_attr( $item_id ); ?>][<?php echo esc_attr( $key ); ?>][value]" class="value"><?php echo esc_textarea( $epo['value'] ); ?></textarea>
                </div>
            </div>
		<?php } ?>
    </td>
    ...
    <td class="item_cost" width="1%" data-sort-value="<?php echo esc_attr( $order->get_item_subtotal( $item, FALSE, TRUE ) ); ?>">
		...
		if ( $epo_can_show_order_price ) {
			if ( $epo['quantity'] <= 0 ) {
				...
				if ( $epo_edit_cost ) {
					echo '<div class="edit"><input type="' . esc_attr( $input_type ) . '" name="tm_epo[' . esc_attr( $item_id ) . '][' . esc_attr( $key ) . '][price]" placeholder="0" value="0" data-qty="0" size="4" class="price" /></div>';
				}
			} else {
				echo '<div class="view">' .
				     ( $epo_is_fee
    ...
    <td class="quantity" width="1%">
		...
		<?php if ( $epo_edit_quantity ) { ?>
            <div class="edit">
                ...
                <input type="<?php echo esc_attr( $input_type ); ?>" step="1" min="0" autocomplete="off" name="tm_epo[<?php echo absint( $item_id ); ?>][<?php echo esc_attr( $key ); ?>][quantity]" placeholder="0" value="<?php echo esc_attr( $epo['quantity'] ); ?>" data-qty="<?php echo esc_attr( $epo['quantity'] ); ?>" size="4" class="quantity"/>
