...
<div data-uniqid="<?php echo esc_attr( $uniqid ); ?>"
     ...
     class="cpf_hide_element tm-cell <?php echo esc_attr( $column ); ?> cpf-type-<?php echo esc_attr( $type . $divclass . $tm_product_id_class ); ?>"
     <?php 
...
if ( $element != "divider" ) {
	...
	if ( ( ! empty( $title ) && ( $title_position == "left" || $title_position == "right" ) ) && ( ! empty( $description ) && ( $description_position !== "below" ) ) ) {
		echo '<div';		
		echo ' class="tm-description' . ( $description_position == "tooltip" || $description_position == "icontooltipright" || $description_position == "icontooltipleft" ? " tm-tip-html" : "" ) . esc_attr($descriptionclass) .'">';
	...
	if ( $title_position != "disable" ) {
		...
		if ( ! empty( $title ) || ! empty( $required ) || ( ! empty( $description ) && ( $description_position == "icontooltipright" | $description_position == "icontooltipleft" ) ) ) {
			...
			echo ' class="tm-epo-element-label' . esc_attr( $class ) . '">';
...
switch ( $divider ) {
	case "hr":
		echo '<hr class="hr_divider' . esc_attr( $divider_class ) . '">';
	break;
	case "divider":
		echo '<div class="tm_divider' . esc_attr( $divider_class ) . '"></div>';
	break;
	case "padding":
		echo '<div class="tm_padding' . esc_attr( $divider_class ) . '"></div>';
	break;
}
